...
getMCLEsv = function(compLike, selSite, sels, gs, times) {
  # Gets MCLE for location of selected site strength of selection (s), frequency of standing variant (g),
  #   standing time (t), and source population for standing variant model
  #
  # Args: 
  ...
  # Returns:
  #	MCLE for location of selected site, s, g, t, and source population
  maxLocIndex = ceiling(which.max(unlist(compLike)) / (length(sels) * length(gs) * length(times)))
  maxSelIndex = ceiling(which.max(unlist(compLike[[maxLocIndex]])) / (length(gs) * length(times)))
  maxGIndex = ceiling(which.max(unlist(compLike[[maxLocIndex]][[maxSelIndex]])) / (length(times)))
  maxTimeIndex = ceiling(which.max(unlist(compLike[[maxLocIndex]][[maxSelIndex]][[maxGIndex]])))
  ...
  return(cbind(maxLoc, maxSel, maxG, maxTime))
}

getMCLEmig_stagSweeps = function(compLike, selSite, sels, gs, times, sources) {
  # Gets MCLE for location of selected site strength of selection (s), frequency of standing variant (g),
  #   standing time (t), and source population for standing variant model
  #
  # Args: 
